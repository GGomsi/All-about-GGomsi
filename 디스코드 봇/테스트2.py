        ...
        while tank < 2 or deal < 6 or heal < 2:
            ...
            if coin.content == "!참가 탱커":
                ...
                if tank == 2:
                    ...
                    try: 
                        if all_list.index(coin.author.id) == 0 or 1 or 2 or 3 or 4 or 5 or 6 or 7 or 8 or 9:
...
# 랭킹
    if message.content == "!랭킹":
            ...
            msgembed = discord.Embed(title="랭킹", description=f"{MMR_list[1:11].to_string(index=False)}", color=0x4000FF) #큰 제목과 작은 제목을 보여준다
            await message.channel.send(embed=msgembed)
